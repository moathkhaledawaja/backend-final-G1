...
describe('UserRatingService', () => {
  ...
  describe('createUserRating', () => {
    it('should create and return a user rating', async () => {
      ...
      const result = await userRatingService.createUserRating(userId, productId, userRatingData);

      expect(userRatingRepository.create).toHaveBeenCalledWith(expect.any(UserRating));
    ...
    it('should throw an InternalServerError if an error occurs', async () => {
      ...
      (userRatingRepository.create as jest.Mock).mockRejectedValue(new Error('Database error'));

      await expect(userRatingService.createUserRating(userId, productId, userRatingData)).rejects.toThrow(InternalServerError);
  ...
  describe('findUserRatingsByProductId', () => {
    it('should return the average rating for a product', async () => {
      ...
      (userRatingRepository.findAllByProductId as jest.Mock).mockResolvedValue(userRatings);
    ...
    it('should throw an InternalServerError if an error occurs', async () => {
      ...
      (userRatingRepository.findAllByProductId as jest.Mock).mockRejectedValue(new Error('Database error'));

      await expect(userRatingService.findUserRatingsByProductId(productId)).rejects.toThrow(InternalServerError);
      expect(logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe('findUserRatingByUserIdAndProductId', () => {
    it('should return a user rating for the given user and product', async () => {
      ...
      (userRatingRepository.findByUserIdAndProductId as jest.Mock).mockResolvedValue(userRating);
    ...
    it('should return null if no rating is found', async () => {
      ...
      (userRatingRepository.findByUserIdAndProductId as jest.Mock).mockResolvedValue(null);
    ...
    it('should throw an InternalServerError if an error occurs', async () => {
      ...
      (userRatingRepository.findByUserIdAndProductId as jest.Mock).mockRejectedValue(new Error('Database error'));

      await expect(userRatingService.findUserRatingByUserIdAndProductId(userId, productId)).rejects.toThrow(InternalServerError);
      expect(logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe('updateUserRating', () => {
    it('should update and return the updated user rating', async () => {
      ...
      updatedUserRating.productId = productId;
        ...
        (userRatingRepository.update as jest.Mock).mockResolvedValue(updatedUserRating);

      const result = await userRatingService.updateUserRating(userId, productId, userRatingData);
    ...
    it('should throw an InternalServerError if an error occurs', async () => {
      ...
      (userRatingRepository.update as jest.Mock).mockRejectedValue(new Error('Database error'));

      await expect(userRatingService.updateUserRating(userId, productId, userRatingData)).rejects.toThrow(InternalServerError);
      expect(logger.error).toHaveBeenCalled();
    });
  });
});
