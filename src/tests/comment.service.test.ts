...
describe('CommentService', () => {
  ...
  describe('getCommentsByProductId', () => {
    ...
    it('should throw an error if there is an issue retrieving comments', async () => {
      ...
      commentRepository.findByProductId.mockRejectedValue(new Error('Database error'));

      await expect(commentService.getCommentsByProductId(productId)).rejects.toThrow(
  ...
  describe('getCommentById', () => {
    it('should return a comment by id', async () => {
      const commentId = 1;
      const comment = { toJSON: jest.fn().mockReturnValue({}) } as unknown as Comment;
  ...
  describe('updateComment', () => {
    it('should update a comment and return the updated comment', async () => {
      ...
      const updatedComment = { toJSON: jest.fn().mockReturnValue({}) } as unknown as Comment;
    ...
    it('should throw an error if there is an issue updating the comment', async () => {
      ...
      await expect(commentService.updateComment(commentId, userId, data)).rejects.toThrow(
