...
describe("CommentService", () => {
  ...
  describe("createComment", () => {
    it("should create and return a comment when the product exists", async () => {
      ...
      expect(productRepository.GetProduct).toHaveBeenCalledWith(commentData.productId);
    ...
    it("should throw an InternalServerError if an error occurs", async () => {
      ...
      (productRepository.GetProduct as jest.Mock).mockRejectedValue(new Error("Database error"));

      await expect(commentService.createComment(userId, commentData)).rejects.toThrow(InternalServerError);
  ...
  describe("getCommentsByProductId", () => {
    it("should return a list of comments for a product", async () => {
      ...
      (commentRepository.findByProductId as jest.Mock).mockResolvedValue(comments);
    ...
    it("should throw an InternalServerError if an error occurs", async () => {
      ...
      (commentRepository.findByProductId as jest.Mock).mockRejectedValue(new Error("Database error"));

      await expect(commentService.getCommentsByProductId(productId)).rejects.toThrow(InternalServerError);
    });
  });
  ...
  describe("getCommentById", () => {
    it("should return a comment by its ID", async () => {
      const commentId = 1;
      const comment: CommentDTO = { id: 1, userId: 1, productId: 123, content: "Great product!" };
    ...
    it("should throw an InternalServerError if an error occurs", async () => {
      ...
      (commentRepository.findById as jest.Mock).mockRejectedValue(new Error("Database error"));

      await expect(commentService.getCommentById(commentId)).rejects.toThrow(InternalServerError);
      expect(logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("updateComment", () => {
    it("should update and return the updated comment", async () => {
      ...
      const updatedComment: CommentDTO = { id: 1, userId: 1, productId: 123, content: "Updated content" };
    ...
    it("should throw an InternalServerError if an error occurs", async () => {
      ...
      (commentRepository.update as jest.Mock).mockRejectedValue(new Error("Database error"));

      await expect(commentService.updateComment(commentId, userId, updateData)).rejects.toThrow(InternalServerError);
      expect(logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("deleteComment", () => {
    it("should delete the comment and return true", async () => {
      ...
      (commentRepository.findByUserIdAndId as jest.Mock).mockResolvedValue({ id: commentId, userId });
      (commentRepository.delete as jest.Mock).mockResolvedValue(true);

      const result = await commentService.deleteComment(commentId, userId);

      expect(commentRepository.findByUserIdAndId).toHaveBeenCalledWith(userId, commentId);
    ...
    it("should return false if the comment is not found", async () => {
      ...
      (commentRepository.findByUserIdAndId as jest.Mock).mockResolvedValue(null);
    ...
    it("should throw an InternalServerError if an error occurs", async () => {
      ...
      (commentRepository.findByUserIdAndId as jest.Mock).mockRejectedValue(new Error("Database error"));

      await expect(commentService.deleteComment(commentId, userId)).rejects.toThrow(InternalServerError);
      expect(logger.error).toHaveBeenCalled();
    });
  });
});
